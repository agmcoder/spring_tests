...
public class E2ECreateFilmTest {
	...
	public void createfilm() throws Exception {
		...
		try {
			...
			String titleAndYear = doc.getElementsByTagName("h1").item(0).getTextContent().trim();
			aux =  titleAndYear.split("\\(");
			
			assertEquals(title, aux[0]);
			assertEquals(year, Integer.parseInt(aux[1].replace(")", "")));
		} catch (Exception e) {
			e.printStackTrace();
		}
